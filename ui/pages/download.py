...
def render():
    ...
    # PDF preview
    with st.expander("Preview your Tax Roadmap"):
        filing_status = st.session_state.get("filing_status", "Single")
        citizenship = st.session_state.get("citizenship_status", "US citizen")
        required_forms = [f for f in forms if f.get("required")]
        optional_forms = [f for f in forms if not f.get("required")]

        st.markdown(f"**Tax Year:** {selected_year}")
        st.markdown(f"**Filing Status:** {filing_status}")
        st.markdown(f"**Residency:** {citizenship}")

        st.markdown("---")
        st.markdown(f"**Required Forms ({len(required_forms)}):** " + ", ".join(f["form"] for f in required_forms))
        if optional_forms:
            st.markdown(f"**Optional Forms ({len(optional_forms)}):** " + ", ".join(f["form"] for f in optional_forms))

        st.markdown("---")
        for form_name, lines in list(guidance.items())[:1]:
            st.markdown(f"**Sample — {form_name} guidance:**")
            for g in lines[:3]:
                st.markdown(f"- **{g['line']}** ({g.get('label', '')}): {g.get('plain_english', '')[:120]}...")

        if len(guidance) > 1:
            remaining = sum(len(v) for k, v in list(guidance.items())[1:])
            st.caption(f"+ {remaining} more lines across {len(guidance) - 1} additional form(s) in the full PDF.")
